"""
Modern MainWindow with SQLite Database Integration
CustomTkinter ve SQLite ile modern arayüz ve kalıcı veri saklama
"""
...
class ModernMainWindowDB:
    """
    Modern ana pencere sınıfı - SQLite ile
    CustomTkinter ve SQLite veritabanı entegrasyonu
    """
    
    def __init__(self):
        """
        ModernMainWindowDB constructor
        """
        # Ana pencere
        self.root = ctk.CTk()
        self.root.title("✨ Günlük Alışkanlık Takip Uygulaması (SQLite)")
        self.root.geometry("1200x800")
        self.root.resizable(True, True)
        
        # Veri yönetimi
        self.data_manager = DataManagerDB()
        ...
        # Arayüz bileşenleri
        self.tabview = None
        self.habits_cards_frame = None
        self.stats_scroll_frame = None
    ...
    def create_settings_tab(self):
        ...
        title_label = ctk.CTkLabel(
            tab_frame,
            text="⚙️ Uygulama Ayarları (SQLite)",
            font=ctk.CTkFont(size=20, weight="bold")
        )
        title_label.pack(pady=20)
        ...
        db_info_frame = ctk.CTkFrame(tab_frame)
        db_info_frame.pack(fill="x", padx=20, pady=10)
        
        db_info_label = ctk.CTkLabel(
            db_info_frame,
            text="📊 Veritabanı Bilgileri:",
            font=ctk.CTkFont(size=14, weight="bold")
        )
        db_info_label.pack(pady=10)
        
        # Veritabanı bilgilerini göster
        self.show_database_info(db_info_frame)
    ...
    def show_database_info(self, parent):
        ...
        try:
            ...
            info_text = f"""
📁 Veritabanı Yolu: {db_info.get('file_path', 'Bilinmiyor')}
📏 Dosya Boyutu: {db_info.get('file_size', 0)} bytes
👥 Kullanıcı Sayısı: {db_info.get('users_count', 0)}
🎯 Alışkanlık Sayısı: {db_info.get('habits_count', 0)}
📝 Kayıt Sayısı: {db_info.get('records_count', 0)}
✅ Veritabanı Durumu: {'Var' if db_info.get('exists', False) else 'Yok'}
            """
            ...
            error_label = ctk.CTkLabel(
                parent,
                text=f"❌ Veritabanı bilgileri alınamadı: {e}",
    ...
    def refresh_habits_list(self):
        ...
        try:
            ...
            user_habits = self.data_manager.get_user_habits(self.current_user.user_id, True)
    ...
    def update_statistics_display(self):
        ...
        try:
            ...
            for habit in user_habits:
                ...
                # İstatistikler
                stats = self.data_manager.get_habit_statistics(habit.habit_id, 30)
                ...
                progress_bar = ctk.CTkProgressBar(left_col, width=150, height=20)
                ...
                if completion_rate >= 80:
                    ...
                    progress_bar.configure(progress_color="#FF9800")  # Orange
                else:
                    progress_bar.configure(progress_color="#F44336")  # Red
                
                ctk.CTkLabel(left_col, text=f"%{completion_rate}", 
                ...
                ctk.CTkLabel(middle_col, text=f"{stats.get('current_streak', 0)} gün", 
                ...
                ctk.CTkLabel(right_col, text=f"{stats.get('total_completions', 0)}", 
        ...
        def register_user():
            ...
            if user.register(username, email, password):
                ...
                if self.data_manager.save_user(user):
                    ...
                    messagebox.showerror("Hata", "Kullanıcı veritabanına kaydedilemedi!")
            else:
    ...
    def show_add_habit_dialog(self):
        ...
        def add_habit():
            ...
            # Alışkanlık adı validasyonu
            if not name:
                messagebox.showwarning("Uyarı", "⚠️ Alışkanlık adı boş olamaz!\n\nLütfen bir alışkanlık adı girin.")
                name_entry.focus()
                return
            
            # Alışkanlık adı uzunluğu kontrolü
            if len(name) < 2:
            ...
            if len(name) > 50:
                messagebox.showwarning("Uyarı", "⚠️ Alışkanlık adı çok uzun!\n\nLütfen en fazla 50 karakter girin.")
                name_entry.focus()
                return
            
            # Hedef frekans validasyonu
            try:
                frequency = int(freq_entry.get())
                if frequency < 1:
                    messagebox.showwarning("Uyarı", "⚠️ Hedef frekans 1'den küçük olamaz!\n\nLütfen en az 1 gün girin.")
                    freq_entry.focus()
                    return
                if frequency > 365:
                    messagebox.showwarning("Uyarı", "⚠️ Hedef frekans çok yüksek!\n\nLütfen en fazla 365 gün girin.")
                    freq_entry.focus()
                    return
            except ValueError:
            ...
            if description and len(description) > 200:
                messagebox.showwarning("Uyarı", "⚠️ Açıklama çok uzun!\n\nLütfen en fazla 200 karakter girin.")
                desc_entry.focus()
                return
            ...
            if habit.create_habit(name, description, frequency, self.current_user.user_id):
                # Veritabanına kaydet
                if self.data_manager.save_habit(habit):
                    self.habits.append(habit)
                    self.refresh_habits_list()
                    dialog.destroy()
                    messagebox.showinfo("Başarı", "Alışkanlık başarıyla eklendi!")
                else:
                    messagebox.showerror("Hata", "Alışkanlık veritabanına kaydedilemedi!")
            else:
    ...
    def save_data(self):
        """
        Verileri kaydeder (SQLite)
        """
        try:
            # SQLite'de veriler zaten otomatik olarak kaydedilir
            # Bu fonksiyon sadece bilgilendirme amaçlıdır
            messagebox.showinfo("Bilgi", "Veriler SQLite veritabanına otomatik olarak kaydedilir!\n\nVeritabanı bilgileri için Ayarlar sekmesine bakın.")
        except Exception as e:
            messagebox.showerror("Hata", f"İşlem hatası: {e}")
    
    def load_data(self):
        """
        Verileri yükler (SQLite)
        """
        try:
            # Kullanıcıları yükle
            ...
            self.habits = self.data_manager.load_habits()
            
            # Kayıtları yükle
            self.records = self.data_manager.load_habit_records()
            
            # İlk kullanıcıyı mevcut kullanıcı yap
            if self.users and not self.current_user:
                self.current_user = self.users[0]
                
            print(f"SQLite veritabanından veriler yüklendi:")
            print(f"  Kullanıcılar: {len(self.users)}")
            print(f"  Alışkanlıklar: {len(self.habits)}")
            print(f"  Kayıtlar: {len(self.records)}")
    ...
    def create_backup(self):
        ...
        try:
            if self.data_manager.create_backup(self.users, self.habits, self.records):
                messagebox.showinfo("Başarı", "Veritabanı yedeği başarıyla oluşturuldu!")
            else:
                messagebox.showerror("Hata", "Veritabanı yedeği oluşturulamadı!")
        except Exception as e:
